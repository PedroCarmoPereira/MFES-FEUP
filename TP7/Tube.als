...
fact {
  ...
  // c) the Circle line forms a circle
  all s : CircleStation | one s.circle and CircleStation in s.^(circle)

  // d) Jubilee is a straight line starting at Stanmore
  all j : JubileeStation | lone j.jubilee
  JubileeStation = Stanmore + Stanmore.^(jubilee)

  // e) there's a station between Stanmore and BakerStreet
  some s : Station | s in (Stanmore.^(jubilee) & BakerStreet.^(circle))
